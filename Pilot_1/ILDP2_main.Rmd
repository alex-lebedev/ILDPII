...
```{r, echo = T}
load('/Users/alebedev/GitHub/ILDPII/Pilot_1/data/rdata.rda')
head(rdata)
```
...
dataList <- list(
  N             = 1,
  T             = 80,
  response      = as.numeric(sqrt(rdata$gsr)),
...
for (i in 1:10){
  rdata[which(rdata$block==2 & rdata$subject==i)[1],'revtrial'] <- 1
  rdata[which(rdata$block==3 & rdata$subject==i)[1],'revtrial'] <- 1
  rdata[which(rdata$block==4 & rdata$subject==i)[1],'revtrial'] <- 1
}
...
### Run Model:
mysamples <- stan(file='/Users/alebedev/GitHub/ILDP2_pilot/stan_models/RW_instr_multipleSubj.stan',   
                  data=dataList, 
                  #                pars=parameters,
                  iter=3000, 
                  chains=4, 
