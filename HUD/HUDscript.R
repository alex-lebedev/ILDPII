...
SCREEN_df_tot <- ALLSCR
ALLFU_df_tot <-  ALLFU_wDemogr[!is.na(ALLFU_wDemogr$CONS5),]

#subset young adults
SCREEN_df <- SCREEN_df_tot[(SCREEN_df_tot$age<36 & SCREEN_df_tot$age>17),]
SCREEN_df <-  SCREEN_df[!is.na(SCREEN_df$drug_psychedelics),]
ALLFU_df <- ALLFU_df_tot[(ALLFU_df_tot$age<36& ALLFU_df_tot$age>17),]
ALLFU_df <-  ALLFU_df[!is.na(ALLFU_df$CONS5),]
...
ALLFU_df_noPsych <- subset(ALLFU_df, ALLFU_df$PsychDiagAny==0)
SCREEN_df_Psych <- subset(SCREEN_df, SCREEN_df$PsychDiagAny==1)
ALLFU_df_Psych <- subset(ALLFU_df, ALLFU_df$PsychDiagAny==1 )
...
write.xlsx2(descrTable, file='/Users/alebedev/Downloads/descrTable.xlsx')
